...
def _determine_base_url(document, page_url):
    """Determine the HTML document.txt's base URL.

    This looks for a ``<base>`` tag in the HTML document.txt. If present, its href
    attribute denotes the base URL of anchor tags in the document.txt. If there is
    no such tag (or if it does not have a valid href attribute), the HTML
    file's URL is used as the base URL.

    :param document: An HTML document.txt representation. The current
        implementation expects the result of ``html5lib.parse()``.
    :param page_url: The URL of the HTML document.txt.
    """
